...
then
  ...
  case $INPUT_STRING in
	...
	9)
		...
		then
			echo "Pausando o Container "$container$"."
			lxc stop $container
			echo "O Container "$container$" foi pausado."
			pauseStart="Iniciar"
		else
			echo "Inicializando o Container "$container$"."
			lxc start $container
			echo "O Container "$container$" foi inicializado."
			pauseStart="Pausar"		
		fi
		;;				
	0)
